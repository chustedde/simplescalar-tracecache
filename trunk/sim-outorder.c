...
/*TU Pre-processor Constants*/
#define TRACE_CACHE_SIZE	64			//sets in the direct mapped trace cache
#define TRACE_RATIO 		7 * INSTS_PER_TRACE	//sampling rate
#define INSTS_PER_TRACE 	16					//number of instructions per trace
#define TRACE_WRITE_FILE   "TraceResults.txt"
#define BAD_TRACE_THRESHOLD  32		//a counter to determine when to end a trace build for bad trac
...
static int next_build_branch_index = 0; //index of next branch during build in flags and b_pc
static int next_using_branch_index = 0;		//index of next branch during using in flags and b_pc
	  ...
	  && (MD_OP_FLAGS(rs->op) & F_CTRL))
	{
	  bpred_update(pred,
		...
		if(!trace_being_formed)
			tc_build_index = rs->PC % TRACE_CACHE_SIZE;
		
	      /*TU start/continue building trace*/
      if(!using_trace_cache || (tc_build_index != tc_using_index))
      {
      	...
      	if(!count_to_next_trace-- || trace_being_formed)
      	{
      		...
      		else
      		{
				  ...
				  {
				  		build_buffer.flags[next_build_branch_index] = (rs->next_PC != (rs->PC + sizeof(md_inst_t)));
				  		prev_branch_was_taken = (rs->next_PC != (rs->PC + sizeof(md_inst_t)));
				  		target_prev_taken_branch = rs->next_PC;
				  		build_buffer.b_pc[next_build_branch_index] = rs->PC;
				  		build_buffer.mask++;
					  	next_build_branch_index++;
...
ruu_dispatch(void)
{
  ...
  while (/* instruction decode B/W left? */
	      ...
	      && !OPERANDS_READY(last_op.rs)))
	{
      ...
      /* get the next instruction frofm the IFETCH -> DISPATCH queue */
  ...
  for (i=0, branch_cnt=0;
	  ...
	  && !(fetch_regs_PC & (sizeof(md_inst_t)-1)))
	{
	  ...
	  MD_FETCH_INST(inst, mem, fetch_regs_PC);
		...
		//get trace line TU//
		if(!using_trace_cache)
			tc_using_index = search_tc();
		
		//check that we should fetch from trace cache otherwise get inst from cache TU//
		if(using_trace_cache && using_pc_index < tc[tc_using_index].n_insts)
		{
			lat = cache_il1_lat;
			if(using_pc_index == 0)
			{		
      ...
      else
	{
	 /*  no predictor, just default to predict not taken, and
      ...
      if(using_trace_cache)
      {
      	...
      	if (MD_OP_FLAGS(op) & F_CTRL)
      	{
			   fetch_data[fetch_tail].pred_PC = fetch_pred_PC = tc[tc_using_index].pred_pc[next_using_branch_index];
      		next_using_branch_index++;
      	}
      	else
      	{
...
int search_tc()
{
	...
	struct bpred_update_t *directory_ptr;
	...
	if(tc[i].valid && fetch_regs_PC == tc[i].pc[0])
	{
		...
		for(j = 0; j < tc[i].mask; j++)
		{
			...
			{
				directory_ptr->dir.ras = FALSE;
			  directory_ptr->pdir1 = NULL;
			  directory_ptr->pdir2 = NULL;
			  directory_ptr->pmeta = NULL;
			  ...
			  switch (pred->class) {
					...
					if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
				{
				  ...
				  bimod = bpred_dir_lookup (pred->dirpred.bimod, tc[i].b_pc[j]);
				  twolev = bpred_dir_lookup (pred->dirpred.twolev, tc[i].b_pc[j]);
				  meta = bpred_dir_lookup (pred->dirpred.meta, tc[i].b_pc[j]);
				  directory_ptr->pmeta = meta;
				  directory_ptr->dir.meta  = (*meta >= 2);
				  directory_ptr->dir.bimod = (*bimod >= 2);
				  directory_ptr->dir.twolev  = (*twolev >= 2);
				  if (*meta >= 2)
					 {
						directory_ptr->pdir1 = twolev;
						directory_ptr->pdir2 = bimod;
					 }
				  else
					 {
						directory_ptr->pdir1 = bimod;
						directory_ptr->pdir2 = twolev;
					 }
				}
					break;
				 case BPred2Level:
				...
				{
				  directory_ptr->pdir1 =
					 bpred_dir_lookup (pred->dirpred.twolev, tc[i].b_pc[j]);
				}
					break;
				 case BPred2bit:
					if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
				{
				  directory_ptr->pdir1 =
					 bpred_dir_lookup (pred->dirpred.bimod, tc[i].b_pc[j]);
				}
					break;
				 default:
					panic("bogus predictor class");
			 }
			 if(pred->class == BPredTaken)
				prediction = 2;
			else if(pred->class == BPredNotTaken)
				prediction = 0;
			else
			if(*(directory_ptr->pdir1) >= 2)
				/* taken */ prediction = 2;
			else
	      	/* not taken */ prediction = 0;
	    	}
			...
			{
				if((tc[i].flags[j] == 1 && prediction < 2) || (tc[i].flags[j] == 0 && prediction >= 2))
					return -1;
		...
		next_using_branch_index = 0;
		using_pc_index = 0;
		if(trace_being_formed && tc_build_index == i)
		{
			next_build_branch_index = 0;
			trace_being_formed = 0;
  			prev_branch_was_taken = -1;
